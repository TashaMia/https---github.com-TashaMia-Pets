...
const petSlider = pets.slice()


let cardsPets = document.querySelector('#cards');
let card = document.createElement('div');
card.className = 'card';
cardsPets.append(card);
let countSlider = 0;
  document.querySelector('#scroll-forward').addEventListener('click', function scrollSlides() {
    cardsPets.scrollLeft += 320;
    countSlider++;
    console.log(countSlider)
    // if(countSlider == 7){
    //     let removed = petSlider.splice(0, 7)
    //     console.log(petSlider)
    //     petSlider.splice(1, 0, removed)
    //     console.log(petSlider)
  ...
  document.querySelector('#scroll-back').addEventListener('click', function scrollSlides() {
    ...
    // createCard()
    cardsPets.scrollLeft -= 320;
    // console.log(cardsPets.scrollLeft)
    // if(cardsPets.scrollLeft == 0){
    //     cardsPets.scrollLeft = 1763
    // } 
...
document.querySelector('#burger').addEventListener('click', function actionSideBar(){
    if(document.querySelector('#burger').style.transform == 'none'){
        ...
        document.querySelector('#nav-420px').style.display= 'flex';
    }else{
        document.querySelector('#burger').style.transform = 'none'
        document.querySelector('#nav-420px').style.display= 'none'
...
modalOpen.forEach((e)=>{e.addEventListener('click', ()=>{
    ...
    modals.append(modal);
     ...
     modal.innerHTML = `
     <img src = ${pets[e.id].img} width="230px" height="260px">
     <div id="modal-info">
     <p>Name: ${pets[e.id].name}</p>
     <p>Type: ${pets[e.id].type}</p>
     <p>Species: ${pets[e.id].species}</p>
     <p>Desctription: ${pets[e.id].desctription}</p>
     </div>
     `
